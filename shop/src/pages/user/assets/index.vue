    ...
    <main class="mx-auto w-full max-w-5xl px-4">
      <TransitionGroup name="list" tag="div" class="space-y-3">
        <div v-for="l in list" :key="l.id" class="card flex items-center gap-3 p-4">
          <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full" :class="l.change_type === 1 ? 'text-success' : 'text-danger'" :style="{ background: `color-mix(in srgb, ${l.change_type === 1 ? 'var(--success)' : 'var(--danger)'} 10%, transparent)` }">
            <AppIcon :name="l.change_type === 1 ? 'plus' : 'minus'" :size="20" />
          </div>
          <div class="min-w-0 flex-1">
            ...
            <p class="mt-0.5 flex items-center gap-1 text-xs text-fg-3">
              <AppIcon :name="l.credit_type === 0 ? 'star' : 'points'" :size="12" />{{ l.created_at }}
            </p>
          </div>
          <div class="shrink-0 text-right">
            <p class="font-bold tabular-nums" :class="l.change_type === 1 ? 'text-success' : 'text-fg'">
              {{ l.change_type === 1 ? '+ ' : '- ' }}{{ l.change_amount }}
            </p>
...
function loadList() {
  ...
  api.getAssetList(page.value, 20, type.value).then((res) => {
    if (seq !== requestSeq)
      return
    if (res.code === 0) {
