...
// @Param data body input.ProductShopListPageReq true "分页参数"
...
func (h *Handler) ShopListPage(c *gin.Context) {
	...
	// 获取参数
	var req input.ProductShopListPageReq
	if code, ok, err := handler.BindAndValidate(c, &req); !ok {
		handler.ErrorLog(logger.ProductLogger, "ShopListPage 参数异常", code, err)
...
func (h *Handler) ShopDetail(c *gin.Context) {
	...
	// 返回结果
	response.Success(c, lang, toProductDetailResp(svcResp, false))
}
...
func (h *Handler) UpdateEnable(c *gin.Context) {
	ctx := c.Request.Context()
	lang := i18n.GetLang(ctx)
	// 获取管理员ID
	adminInfo, ok := handler.GetAdminInfo(c)
	...
	if code, ok, err := handler.BindAndValidate(c, &req); !ok {
		handler.ErrorLog(logger.ProductLogger, "UpdateEnable 参数异常", code, err)
		response.Error(c, lang, code)
		return
	}
	// 执行请求
...
func (h *Handler) AdminDetail(c *gin.Context) {
	...
	var req input.ProductDetailReq
	if code, ok, err := handler.BindAndValidate(c, &req); !ok {
		handler.ErrorLog(logger.ProductLogger, "AdminDetail 参数异常", code, err)
		response.Error(c, lang, code)
		return
	}
	// 执行请求
