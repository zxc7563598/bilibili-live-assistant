...
import (
	...
	altchaSvc "github.com/zxc7563598/GoAdminKit/internal/service/altcha"
	"go.uber.org/zap"
)
...
func (h *Handler) Login(c *gin.Context) {
	...
	// 验证码校验
	if errCode, err := h.altchaSvc.VerifySolution(ctx, req.Captcha); errCode != 0 {
		handler.ErrorLog(
			logger.AdminLogger,
			"altchaSvc.VerifySolution 调用失败",
			errCode,
			err,
		)
	...
	// 执行请求
	svcResp, errCode, err := h.adminSvc.Login(ctx, req.Username, req.Password)
	if errCode != 0 {
		handler.ErrorLog(
			logger.AdminLogger,
			"adminSvc.Login 调用失败",
