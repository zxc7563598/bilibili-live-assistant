...
type RoomConfigResp struct {
	...
	ConsumeRewardEnabled string `json:"consume_reward_enabled" example:"0"`
	// 奖励类型, 0-星光, 1-积分
	RewardType string `json:"reward_type" example:"1"`
	// 消费电池转换倍率, 奖励设置为按消费电池发放时生效
	ConsumeBatteryRate string `json:"consume_battery_rate" example:"0"`
	// 开通舰长奖励数量, 奖励设置为按开通航海类型发放时生效
	CaptainRewardAmount string `json:"captain_reward_amount" example:"0"`
	// 开通提督奖励数量, 奖励设置为按开通航海类型发放时生效
	CommanderRewardAmount string `json:"commander_reward_amount" example:"0"`
	// 开通总督奖励数量, 奖励设置为按开通航海类型发放时生效
	GovernorRewardAmount string `json:"governor_reward_amount" example:"0"`
}
...
type ReplyItem struct {
	...
	// 关键词匹配策略
	KeywordMatchPolicy string `json:"keyword_match_policy"`
	// 安全词列表
	SafeWord []string `json:"safe_word"`
	// 安全词匹配策略
	SafeWordMatchPolicy string `json:"safe_word_match_policy"`
	// 是否禁言发送者, 0-否, 1-是
